      ...
      namelist / topnml / ds, du, tanl, rnml, stl1, stl2
     &  ,luout, fileout
     &  ,olam
     &  ,wbd, sbd, dlon, dlat
     &  ,idia, jdia, id, jd
...
!================================
     ...
     &             lons,lone,lats,late,demfile,okx,oky
      ...
      write(6,*)"rmsk"
      do j=49,96
       write(6,'(48i2)')(nint(almsk(i,j)),i=1,48)
      enddo
      write(6,*)"zss"
      do j=49,96
...
!       this checks for non-data points, and inserts a neighbouring value
      ...
      write(luout,'(i3,i4,2f6.2,f6.3,f8.0,''  orog-mask-var'')')
...
! convert to "double" (i,j) notation
          lcj=lcj+nface*il
          !write(6,*)i,j,aglon,aglat,lci,lcj
          zsmax=max(zsmax,real(zi(i)))
          zsmin=min(zsmin,real(zi(i)))
...
! sum of squares for sd calc.
             ...
             if ( debug ) then
                if ( lci.eq.idia .and. lcj.eq.jdia ) then
                  write(6,'("lci,lcj,i,zi(i),zs,zss(),almsk(), inum()="
     &           ,4i6,3f10.3,i6)') lci,lcj,i,zi(i),zs,zss(lci,lcj)
     &                       ,almsk(lci,lcj), inum(lci,lcj)
                endif  ! selected points only
             endif  ! debug
...
!#######################################################################
        if (mod(j,100).eq.0) then
         ...
         write(6,*)"j,lon,lat,zsmin,zsmax=",j,aglon,aglat,zsmin,zsmax
!#######################################################################
      enddo ! j
!#######################################################################
