      program terread
      parameter ( mx=3360, my=2800 )
      real blon,blat,dl,elon,elat
      integer*2 izout(mx*my)
      real zout(mx*my)
      character*(80) file
      ...
      read(13) nx,ny,blon,blat,dl
      elon=blon+(nx-1)*dl
      elat=blat+(ny-1)*dl
      write(6,*) nx,ny,dl
      write(6,*) "blon/lat=",blon,blat
      write(6,*) "elon/lat=",elon,elat
      !read(13) (izout(i),i=1,nx*ny)
      !ix=-1
      !in=99999999
      !do i = 1 , nx*ny
      !  ix = max(ix,izout(i))
      ...
      !  zout(i)=izout(i)
      !enddo
      !write(6,*)"all done x,n=",ix,in
      !call amap (zout, nx, ny, 'zs(m)', 100., 0. )
      !!subroutine amap ( w, im, jm, label, cinti, base )
      !call ncdf_setup("t.nc",idnc,3,nx,ny,1,20020101,0000)
      !call nc2out(zout,nx,ny,1,1.,idnc,"zs","zs","m",-100.,5000.)
!
!      call ncsnc(idnc,ier)
...
* data variables
      real data(il,jl)
* packing variables
      integer*2 ipack(3360*2800)
      integer*2 vrange(2)
      data vrange/-32500,32500/
* dim variables
      real xdim(3360),ydim(2800)
...
c create variable using short name
        ...
        if ( debug ) print *,'time id_data,sn,ier=',id_data,sname,ier
...
c give it a fill value
        call ncapt(idnc,id_data,'_FillValue',NCSHORT,1,vrange(1),ier)
...
c define valid scaled max/min variables
        ...
        call ncapt(idnc,id_data,'valid_range',ncshort,2,vrange,ier)
